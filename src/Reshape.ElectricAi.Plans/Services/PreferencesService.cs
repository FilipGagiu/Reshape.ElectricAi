...
{
    private readonly IRepository<UserPreferences> _repository = repository;

    public async Task<PreferencesDto> GetAsync(Guid userId, CancellationToken cancellationToken)
    {
        var entity = await _repository.FirstOrDefaultAsync(new UserPreferencesWithChildrenSpec(userId), cancellationToken);
        return entity.ToDto();
    }
